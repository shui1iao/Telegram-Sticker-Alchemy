...
- **私聊自动转换，群聊显式触发**：私聊可以直接发媒体；群里需要回复媒体发送命令或 @bot，避免普通贴纸/图片刷屏。
...
私聊里也可以直接把图片、贴纸、GIF、短视频发给 Bot，它会自动选择合适的转换方式。

群聊里为了避免刷屏，不会自动处理普通媒体；请回复媒体后发送 `/pts@你的Bot用户名`、`/gif@你的Bot用户名`、`/stp@你的Bot用户名`，或直接回复媒体 @bot。若 bot 收不到被回复媒体，需要在 @BotFather 关闭隐私模式，或把 bot 设为群管理员，然后重新发送媒体再回复。
